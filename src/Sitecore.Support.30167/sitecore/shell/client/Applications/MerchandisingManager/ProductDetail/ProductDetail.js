    ...
    var ProductDetail = cbp.extend({
        ...
        initialized: function () {
            ...
            if (productId) {
                ...
                if (!this.isCommerceFoundationEnabled()) {
				...
				// #30167: Get multiple choice fields
				$.ajax({
					url: "/sitecore/shell/commerce/merchandising/CommerceSearchSupport/GetMultipleChoiceFieldValuesByItemId",
					type: "POST",
					...
					success: function (data) {
						self.setupMultipleChoiceFieldComboBoxes(data);
					}
				});
				...
				var templateId = CommerceUtilities.loadPageVar("template");
				// #30167: Get multiple choice fields
				$.ajax({
					url: "/sitecore/shell/commerce/merchandising/CommerceSearchSupport/GetMultipleChoiceFieldValuesByTemplateId",
					type: "POST",
					headers: this.ajaxToken,
					data: {
						id: templateId,
        ...
        environmentLoaded: function () {
            ...
            if (selectedItem != null) {
                ...
                this.ProductPromotionsDataSource.set("headers", "Environment:" + selectedItem.Name);
		...
		// Start #30167: Multiple Choice Fields
		getMultipleChoiceFieldNames: function() {
			var arrayOfMetadata = this.FormRenderer.get("editFunctionBody");
			var fieldNames = [];
			for(var i = 0; i < arrayOfMetadata.length; i++) {
				var fieldName = arrayOfMetadata[i][0];
				var editFieldName = arrayOfMetadata[i][2];
				var isMultipleField = (editFieldName == "selectedValue");
				if (isMultipleField) {
					// Dropdown
		...
		changeSelectedValueFromNoItemToEmpty: function(dataObject) {
			var multipleFieldNames = this.getMultipleChoiceFieldNames();
		...
		setupMultipleChoiceFieldComboBoxes: function (data) {		
			var arrayOfMetadata = this.FormRenderer.get("editFunctionBody");
			var enumerableFromControls = Enumerable.From(this.Controls);

            for (var i = 0; i < arrayOfMetadata.length; i++) {
                var currentItem = arrayOfMetadata[i];
                var fieldControls = enumerableFromControls.Where(
					function (x) { return (x.name == currentItem[1])&&(x.model.componentName == "ComboBox") }).ToArray();
                if (fieldControls.length > 0) {
					for (var j = 0; j < fieldControls.length; j++) {
						...
						if (field && items) {
							...
							items.push(noItem);
							field.set("displayFieldName", "Name");
							field.set("valueFieldName", "Value");
							field.set("items", items);
							
							if (selectedValue) {
								field.set("selectedValue", selectedValue.toString());
							}
							else {
								field.set("selectedValue", noItem.Value);
							}
						}
					}
                }
		...
		// End #30167: Multiple Choice Fields

        priceCardsComboboxChanged: function () {
            var selectedItem = this.PriceCardsComboBox.get("selectedItem");
            var navigateUrl = "";
            if (selectedItem !== undefined && selectedItem !== null) {
                if (selectedItem.Id == "noPriceCard") {
                    navigateUrl = "";
                    this.PriceCardLink.set("navigateUrl", navigateUrl);
                    this.PriceCardLink.set("text", "");
                } else {
                    navigateUrl = "/sitecore/client/Applications/PricingPromotionsManager/Pages/All%20Books/Price%20Books/PageSettings/Price%20Book/PageSettings/Price%20Card?priceCardId=" + selectedItem.Id;
                    this.PriceCardLink.set("navigateUrl", navigateUrl);
                    ...
                    if (this.priceCardOnload) {
                        ...
                        for (i = 0; i < priceCardItems.length; i++) {
                            if (priceCardItems[i].Name == this.priceCardOnload) {
                                ...
                                this.PriceCardsComboBox.set("selectedItem", selectedItem);
                            }
                        }
        ...
        updateProduct: function () {
            ...
            if (this.PriceCardsComboBox) {
                var priceCard = this.PriceCardsComboBox.get("selectedValue");
                if (priceCard == "noPriceCard") {
                    dataObject.PriceCardName = "";
                } else {
                    dataObject.PriceCardName = this.PriceCardsComboBox.get("selectedItem").Name;
                    $("#productPriceCard").val(this.PriceCardsComboBox.get("selectedItem").Name);
                }
            }
			
			this.changeSelectedValueFromNoItemToEmpty(dataObject); // #30167
